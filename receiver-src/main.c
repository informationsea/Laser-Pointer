...
{
    PORTB = _BV(0);          /* pull up yellow button */
    DDRB = _BV(1) | _BV(2);  /* output two LED */
...
static uchar    keyPressed(void)
{
    ...
    switch (ir_last_command) {
    case IR_COMMAND_NEXT:
        key = 2;
        ir_last_command = IR_COMMAND_NONE;
        break;
    case IR_COMMAND_PREVIOUS:
        key = 3;
        ir_last_command = IR_COMMAND_NONE;
        break;
    }
